import type { ImageMetadata } from "astro";

export const icons = {
  bell: `<svg viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
    <path d="M3 5C3 2.23858 5.23858 0 8 0C10.7614 0 13 2.23858 13 5V8L15 10V12H1V10L3 8V5Z" fill="#000"/>
    <path d="M7.99999 16C6.69378 16 5.58254 15.1652 5.1707 14H10.8293C10.4175 15.1652 9.30621 16 7.99999 16Z" fill="#000"/>
  </svg>`,
  // Ejemplo con asset estático (ImageMetadata)
  whatsapp: (await import("../assets/svg/whatsapp.svg")).default, 
} satisfies Record<string, string | ImageMetadata>;

export type IconName = keyof typeof icons;
